...
import HOD from '../../../components/DashboardComponents/hod/HOD';
...
const HODs = () => {
    ...
    useEffect(() => {
        async function getData() {
            await axios
                .get("http://localhost:8070/hods/view")
                .then((response) => {
    ...
    const handleDelete = async(id) => {
        if (window.confirm("Are you sure that you wanted to delete this hod?")) {
            const response = await axios.delete(`http://localhost:8070/hods/delete/${id}`);
            if (response.status === 200) {
                toast.success("Head of the Department deleted successfully");
                loadingData();
            } else {
        ...
        { /* check if the state is loading otherwise get a blank page because the data is an empty array at the moment of mounting */ } {
            ...
            <MDBContainer style = {{ marginTop: '90px' }} >
                ...
                <MDBTable hover bordered striped style = {{ marginTop: '5px' }} >
                    <caption > List of Head of the Departments </caption> 
