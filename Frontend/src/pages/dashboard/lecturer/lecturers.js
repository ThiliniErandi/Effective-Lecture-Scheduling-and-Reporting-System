...
import Lecturer from '../../../components/DashboardComponents/lecturer/Lecturer';
...
const Lecturers = () => {
    ...
    useEffect(() => {
        async function getData() {
            await axios
                .get("http://localhost:8070/lecturers/view")
                .then((response) => {
    ...
    const handleDelete = async(id) => {
        if (window.confirm("Are you sure that you wanted to delete this lecturer?")) {
            const response = await axios.delete(`http://localhost:8070/lecturers/delete/${id}`);
